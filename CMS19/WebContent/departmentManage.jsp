...
<script type="text/javascript">
	...
	function echo(ec){
		$.ajax({
			...
			success:function(data){
				if(data == "no"){
					$($(ec).next()).html("<font color='red'>学院名称不可为空</font>");
					$($(ec).parent().next().next().find("button")).attr("disabled",true);
				}else if(data == "typeno"){
					$($(ec).next()).html("<font color='red'>学院重复</font>");
					$($(ec).parent().next().next().find("button")).attr("disabled",true);
				}else{
	...
	function sor(so){
		$.ajax({
			...
			success:function(data){
				...
				if(data == "no"){
					$($(so).parent().next()).html("<font color='red'>学院优先级不可为空</font>");
					$($(so).parent().next().parent().next().find("button")).attr("disabled",true);
				}else if(data == "errSortno"){
					$($(so).parent().next()).html("<font color='red'>学院优先级重复</font>");
					$($(so).parent().next().parent().next().find("button")).attr("disabled",true);
				}else{
...
<!-- 部门名称ajax重复判断 -->
			...
			$("#typeName").blur(function(){
				$.ajax({
					...
					success:function(data){
						console.log(data)
						if(data=="ok"){
							$("#errTypeName").html("<font color='red'>学院名称重复</font>");
	...
	function check(){
		var c = true;
		if($("#typeName").val()==""){
			$("#errTypeName").html("<font color='red'>请输入学院名称</font>");
			c=false;
		}
		if($("#sort").val()==""){
                        ...
                        <div class="card ">
                            ...
                            <div class="content">
                                ...
                                <div class="footer">
                                    <hr>
                                    <div class="stats">
                                        <i class="fa fa-history"></i>添加学院
                                        <form action="${pageContext.request.contextPath }/departmentController?flag=addDepartment" method="post" onsubmit="return check()">
                                        <table class="table">
                                            <tr>
                                     
                                                <td>      
					                            <input type="text" class="form-control" name="typeName" id="typeName" placeholder="学院名称"  value="${typeName }">
					                            <span id="errTypeName" style='color:red'>${errTypeName }</span>
					                            </td>
					                            <td>
					                            <div style=" width:130px;">
												<input type="text" class="form-control" placeholder="学院优先级" name="sort" id="sort" value="${sort }" >
                                                ...
                                                <td class="td-actions text-right">
		                          				...
		                          				<button type="submit" id="sb" class="btn btn-info btn-fill pull-right" >添加学院</button>
